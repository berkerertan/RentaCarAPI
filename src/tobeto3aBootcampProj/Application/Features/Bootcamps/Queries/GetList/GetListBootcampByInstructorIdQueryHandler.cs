...
internal class GetListBootcampByInstructorIdQueryHandler : IRequestHandler<GetListBootcampByInstructorIdQuery, BootcampListModel>
{
    ...
    {
        IPaginate<Bootcamp> bootcamp = await _bootcampRepository.GetListAsync(index: request.PageRequest.PageIndex, 
            size: request.PageRequest.PageSize, include: x => x.Include(x => x.Instructor).
